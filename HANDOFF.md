...
- CLI `npm run translate:missing` — пишешь EN, гонишь команду, ~5 мин ревью, commit
...
- 📝 **Контент-каденс** — TZ закладывал 20-30 reviews + 100-200 pSEO. Сейчас 6+4. Чтобы добить — 2-4 статьи в неделю. CLI `npm run translate:missing` после каждой новой EN.
...
# Контент
npm run translate:missing                # все EN без RU twin → переводит CLI'ем
...
- «Напиши новый review/guide» → пиши EN, затем `npm run translate:missing`, отревьюши, commit. Все 7 tools в seed имеют слаги для AffiliateButton (12 после seed reload).
...
- «Почему статья на /ru/X показывает английский?» → Нет RU MDX файла. Проверь `content/reviews/ru/<slug>.mdx`. Если нет — `npm run translate -- --type reviews --slug <slug>`.
