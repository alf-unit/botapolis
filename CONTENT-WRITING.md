...
6. **Gate-строка в `page_publications`** (с 2026-06-04 — флаг
   ...
   - **В drip-очередь** (капельная публикация по расписанию):
     ...
     порядку>}` — Vercel-cron `/api/cron/drip-publish` (`0 8 * * *` UTC = 01:00 LA)
...
### RU style — ЧТО НЕЛЬЗЯ ДЕЛАТЬ (анти-правила, 2026-06-03)

Перевод EN→RU читается **носителем русского языка БЕЗ знания английской
SaaS-терминологии**. Если Shopify-оператор не сидит в англоязычном
SaaS-Twitter — текст должен быть полностью понятен. Калька с английского
не считается переводом.

**1. НЕ оставляй латиницей термины с устоявшимся русским эквивалентом.**
Минимальный список запрещённой кальки (расширяй по мере встречи):

| ❌ Калька / латиница             | ✅ Русский                                |
|----------------------------------|-------------------------------------------|
| AOV                              | средний чек                               |
| churn                            | отток (клиентов)                          |
| retention                        | удержание                                 |
| lookback window                  | период учёта / глубина истории            |
| deflection rate                  | доля автоматически закрытых обращений     |
| overage                          | доплата за превышение                     |
| paywall                          | платная стена / закрытие функции тарифом  |
| baseline                         | базовая величина / точка отсчёта          |
| sunset                           | прекращение поддержки / закрытие продукта |
| passthrough fees                 | проброс комиссии                          |
| platform fee                     | плата за платформу                        |
| mid-cycle                        | в середине биллингового периода           |
| third-party benchmarks           | независимые оценки                        |
| user-reported / operator-reported| по словам пользователей / по отзывам      |
| usage-based pricing              | оплата по факту использования             |
| cap / tier-bumping               | потолок / автоповышение тарифа            |
| onramp                           | заход / начальный этап                    |
| billable contacts                | оплачиваемые контакты                     |
| compound (bill compounds)        | накапливается, нарастает                  |

**2. НЕ используй гибридные «латино-дефисные» конструкции** — это
синтаксически кривo по-русски и читается как машинный перевод.

| ❌ Гибрид с дефисом      | ✅ Нормально по-русски                    |
|--------------------------|-------------------------------------------|
| operator-цитаты          | цитаты операторов / отзывы пользователей  |
| billable-контактов       | оплачиваемых контактов                    |
| growth-фичи              | функции для роста                         |
| operator-reported данные | данные по отзывам операторов              |
| mid-market бренды        | бренды среднего сегмента                  |
| sub-$5M MRR              | до $5M MRR (если MRR оставляем, см. п.4)  |

**3. НЕ перетаскивай служебные английские обороты** буквально.

| ❌                              | ✅                                          |
|---------------------------------|---------------------------------------------|
| «Эта функция doesn't pencil out»| «функция не оправдывается экономически»     |
| «verified 2026-06-03»           | «проверено 2026-06-03»                      |
| «out of the box»                | «из коробки» (или «по умолчанию»)           |
| «hits the bill»                 | «попадает в счёт»                           |
| «pay back»                      | «окупается»                                 |

**4. Латиницу оставляем ТОЛЬКО для:**

- **Brand names**: Klaviyo, Shopify, Mailchimp, ManyChat, Customer Agent,
  Triple Pixel, Moby 2, K:AI и т.п.
- **Технические аббревиатуры без устоявшегося перевода**: ROI, MMM, MTA,
  GMV, SKU, KPI, CRM, SaaS, B2B, DTC, CPC, MRR, ARR. **При первом упоминании
  поясни** в скобках: «ROI (возврат инвестиций)», «MTA (multi-touch
  attribution — атрибуция по нескольким касаниям)».
- **Названия конкретных продуктов/тарифов**: «Plus», «Pro», «Enterprise»,
  «AI Plan» если это собственное имя тарифа, а не его описание.
- **Числа/даты/валюты**: $179, 2026-05-30, 1.49% — формат en-US.
- **Code blocks** (``` ``` ```) и inline backtick (`code`) — не трогать.
- **URLs**: `/tools/`, `/compare/` — без `/ru/` префикса в линках внутри
  тела статьи (i18n routing разрулит на сторону рендера).

**5. Уровень читаемости — главный тест.**

Перед коммитом RU файла: представь Shopify-оператора, который знает русский
свободно, но английский — на уровне «прочитать кнопку Subscribe». Если он
открывает твою RU-страницу и не понимает половину терминов — ты сделал
кальку, а не перевод. Переписывай.

**6. Стилистическая консистентность.**

- **Кавычки**: «русские ёлочки» для блочных цитат и операторских отзывов;
  прямые `"..."` оставляем только внутри кода или URL.
- **Тире**: длинное `—` (em-dash) с пробелами вокруг для вставок и пауз;
  короткое `-` (hyphen) только в составных словах.
- **«ты», не «вы»** — operator-to-operator тон. Vendor-цитаты от компаний
  переводим в нейтральном тоне.
- **Капитализация**: после `—` с маленькой, если это продолжение
  предложения; с большой, если новое предложение.

**Исторический контекст:** в session 3 (2026-06-03) при backfill 15 RU
pricing страниц правила не было — перевод получился transliterated jargon
("operator-цитаты", "billable-контактов", "lookback", "deflection",
"passthrough"). Owner explicit оценил это как «каша из англо-русских слов».
Все 15 RU pricing страниц требуют **ребилда стиля** (помечено в
`/sessions/infra-log.md` open follow-ups). При генерации нового RU контента
— это правило соблюдать с первого захода, чтобы не накапливать долг.
