...
>
> **§3 ПОЛНОСТЬЮ ЗЕЛЁНЫЙ (2026-06-07 session 2)** — все пункты доказаны на preview, см. §3.
> es-scalability ✅ (i18n.locales+='es' → 737 стр Static, /es/* испанский, hreflang авто-es,
> ноль новых route-файлов; es откатан как тест). Live drip-flip ✅ (5 drip-типов 404→200→404
> bare+ru, БД восстановлена). Полный свип ✅ (292/292=200, ноль /en/ утечек). /en/ внутренних
> ссылок ✅ ноль (canonical-страховки достаточно, доп-страж НЕ нужен).
>
> **ОСТАЛОСЬ ТОЛЬКО:** cutover = merge ветки в `main` цельным (НЕ полумиграция) — по явному go
> оператора. Откат после cutover — Vercel «Promote» предыдущего деплоя (секунды). Миграций БД
> нет → откат чисто деплойный.
...
- [x] **bare-EN curl-свип ПОЛНЫЙ** — ✅ весь live-набор из гейт-sitemap: 214 `<loc>` + 78
      /ru-твинов bare-loc = **292 URL, все 200, ноль не-200**. **Ноль `/en/*` в sitemap**
      (0 утечек). Прямой `/en/*` = 200 (дедуп canonical→bare; 301-страж убран — см. баннер).
      Ноль внутренних навигируемых ссылок на `/en/` в коде (`/en/` встречается только в
      комментах и путях файлов контента `content/.../en/...`) → доп-страж НЕ нужен.
- [x] **drip 404→200 reverted-тест по КАЖДОМУ типу** — ✅ ВЫПОЛНЕНО live (gate ON). По одной
      скрытой странице каждого drip-типа: flip visible→revalidate(внутренние `/en`+`/ru`)→
      curl→revert hidden→revalidate→curl. Результат **404→200→404 на bare И /ru** для всех 5:
      tools (`constant-contact`), comparisons (`constant-contact-vs-mailchimp`), pricing
      (`flair-ai`), guides (`yotpo-loyalty-shopify`), best (`best-subscription-app-shopify`).
      `alternatives` — НЕ drip-юнит (enrich-in-place, всегда live, скрытых строк нет → N/A).
      **БД восстановлена** (все 5 пар `(content_type,slug)` обратно `visible_at=NULL`, сверено).
- [x] **live + hidden целы** — ✅ полный свип (ниже) по гейт-sitemap: 214 live `<loc>` +
      /ru-твины = 292 URL, все **200**; скрытые отдают 404 (drip-тест + flair-ai pricing).
...
- [x] **🆕 es-тест масштабируемости** — ✅ ВЫПОЛНЕНО (затем откатан как тест). `i18n.locales
      += 'es'` + `locales/es.json` + запись в `dictionaries` → build **737 стр** (было 494,
      +243 = весь `/es/*` дерево) **Static, ноль новых route-файлов**. Runtime: `/es`,
      `/es/about`, `/es/tools`, `/es/pricing` = 200, `<html lang="es">`, испанский chrome
      (Herramientas/Precios/Comparar/Buscar/Recursos), hreflang **авто-включил es**
      (x-default→bare, en→bare, ru→/ru, es→/es). bare-EN+/ru не задеты. **O(1)-на-язык доказан.**
      es откатан (тест, не релиз). **Оставлено как улучшение:** `buildMetadata` локаль-параметр
      `LanguageCode`→`Locale` (убрал каст). **Замечание для реального es-релиза:** content-
      локализаторы (`localizeTool`, `getMdxContent`, `SavedCard` и пр.) типизированы под
      `LanguageCode` (en|ru) — для es расширить их сигнатуры на `Locale` (или маппить es→en
      fallback). Это типизация контента, не роутинга; на routing O(1) не влияет.
      `EN_SEGMENTS` для es НЕ трогать — `/es/*` обслуживает `[locale]` напрямую (как `/ru/*`).
